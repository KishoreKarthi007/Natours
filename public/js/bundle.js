(() => {
  ...
  var showAlert = (type, message) => {
    ...
    window.setInterval(hideAlert, 5e3);
  };
  ...
  var updateUserSetting = async (data, type) => {
    try {
  ...
  // public/js/stripe.js
  var stripePromise2 = loadStripe("pk_test_51QZbmHSDw5o3u1vwoBInfoVJUt4wxQYsEbP2zPzGUnun74NiVwuCZbbF0Rq3g63wQQCAc8EfOoXqxyroFlAIJdJH00xAJjNcmO");
  var bookTour = async (tourId) => {
    try {
      const session = await axios_default(
        `http://localhost:8000/api/v1/bookings/checkout-session/${tourId}`
      );
      const stripe = await stripePromise2;
